...
Supports the standard NTFS feature set apps depend on: persistent ACLs, long names, named volume identity, and **symbolic links / junctions** (`mklink`, `mklink /D`, `mklink /J`).
...
- **Reparse points** -- symbolic links and junctions are stored per node and resolved
  through WinFsp 2.x's user-mode name-resolution path (`ResolveReparsePoints`). The
  `WinFsp.Native` binding is vendored at `src/WinFsp.Native/` (MIT, exact 0.1.3-pre.1
  source) because the NuGet package leaves that callback slot unwired; all local changes
  are marked `VENDORED DELTA`.
