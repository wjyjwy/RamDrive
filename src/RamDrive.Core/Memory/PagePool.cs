...
public sealed class PagePool : IDisposable
{
    ...
    // Read as a bool via the IsDisposed property at the call sites below.
    ...
    ///
    /// Zero entries are filtered out, matching <see cref="Return"/>: a zero is "no page",
    /// not a page. Counting one and pushing it onto the free stack would make a later
    /// <see cref="Rent"/> hand out a null pointer — and would also decrement the
    /// committed/rented counters for a slot that was never claimed.
    /// </summary>
    public unsafe void ReturnBatch(nint[] pages, int count)
    {
        ...
        _freePages.PushRange(pages, 0, valid);
        Interlocked.Add(ref _rentedCount, -valid);
        Interlocked.Add(ref _committedCount, -valid);
    }

    public void Dispose()
    {
        // Atomic claim: a plain "if (_disposed) return; _disposed = true;" lets two
        // threads both drain _allPages, double-freeing every page.
