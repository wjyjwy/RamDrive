...
public class WinFspRamAdapterFailurePathTests
{
    ...
    public void SetFileSecurity_MalformedDescriptor_ReturnsInvalidSecurityDescriptor_NotException()
    {
        ...
        // STATUS_INVALID_SECURITY_DESCR per the Windows SDK ntstatus.h. Note the value is
        // 0xC000005D, NOT 0xC0000058 — the latter is STATUS_UNKNOWN_REVISION, which this
        // test previously pinned, so it happily passed while the adapter reported the wrong
        // status to WinFsp (and hence the wrong Win32 error to applications).
        status.Should().Be(unchecked((int)0xC000005D));
